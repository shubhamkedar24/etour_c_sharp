...
function PDFview() {
    ...
    useEffect(() => {
        ...
        fetch("https://localhost:44337/api/customers/" + cid)
            .then(res => res.json())
    ...
    useEffect(() => {
        ...
        fetch("https://localhost:44337/api/tourpackages/" + smid)
            .then(res => res.json())
    ...
    useEffect(() => {
        ...
        fetch(`https://localhost:44337/api/passangers/${cid}`)
            .then(res => res.json())
    ...
    function generatepdf() {
        html2canvas(input).then((canvas) => {
            ...
            pdf.save(`Invoice${cid}${smid}${bookingdate}.pdf`);
    ...
    const [cust_id, setCustomer_id] = useState(cid);
    // const [packageid, setPackageid] = useState(bkid);
    const [package_id, setPackage_id] = useState(smid);
    ...
    const handleSubmit = (event) => {
        ...
        fetch("https://localhost:44337/api/bookings/", {
            method: 'POST',
                                    ...
                                    <div className="mb-5 noprint" >
                                        <Form>
                                            ...
                                            <div className="text-center mt-3">
                                                <Button className="btn btn-info mb-2 btn-lg" data-html2canvas-ignore="true" id="btn" type="button" onClick={generatepdf}>Book a Tour
                                                </Button>
                                            </div>
                                        </Form>
                                    </div>
